...
class FibMartingaleStrategy(IStrategy):
    ...
    # 信号重算 — 关键修复
    # ═══════════════════════════════════════
    # 默认 True 时，每根 K 线只在出现瞬间算一次信号，之后 59 秒内信号被 remove_entry_exit_signals 清空，
    # 导致 K 线中途价格穿越斐波那契线时，下单读到的缓存信号已是 0（信号在 UI 上出现但无交易）。
    # 设为 False 让每 5 秒循环都重算信号，K 线中途的穿越也能被捕捉到。
    ...
    # 止盈止损
    # ═══════════════════════════════════════
    minimal_roi = {"0": 0.15}         # 10倍杠杆下盈利 15%（价格波动 1.5%）固定止盈，短线快进快出，不做移动止盈逻辑
    stoploss = -0.25                 # 10倍杠杆下硬止损 -25%（价格波动 -2.5%），不做移动止损或追踪止损逻辑
